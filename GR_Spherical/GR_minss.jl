...
function init!(state::VarContainer{T}, param) where T
    ...
    fΠ(M,r,r̃) = -(f∂t𝜙(M,r,r̃) - fβʳ(M,r,r̃)*fψ(M,r,r̃) )/fα(M,r,r̃)
    #fΠ(M,r,r̃) = 0.
    ...
    f∂tγrr_KS(M,r,r̃) = -8*pi*r(r̃)*fSr(M,r,r̃)*sqrt(fγrr(M,r,r̃))
    ...
    fKrri(M,∂ᵣM,r,r̃)   = -f∂tγrr_KS(M,r,r̃)/fα(M,r,r̃)/2 + fKrr(M,∂ᵣM,r,r̃)
...
function rhs!(dtstate::VarContainer{T},regstate::VarContainer{T}, param::Param{T}, t) where T
    ...
    @. ∂tγrr  = βʳ*∂ᵣγrr + 2*∂ᵣβʳ*γrr - 2*α*Krr

    @. ∂tγθθ  = βʳ*∂ᵣγθθ - 2*α*Kθθ

    @. ∂tKrr  = ( βʳ*∂ᵣKrr - α*∂ᵣfrrr/γrr + 2*α*frrr^2/γrr^2 - 6*α*frθθ^2/γθθ^2
     - α*Krr^2/γrr + 2*α*Krr*Kθθ/γθθ - 8*α*frrr*frθθ/(γrr*γθθ)
     - α*frrr*∂ᵣlnᾶ/γrr - α*∂ᵣlnᾶ^2 - α*∂ᵣ2lnᾶ + 2*∂ᵣβʳ*Krr)

    @. ∂tKθθ  = ( βʳ*∂ᵣKθθ - α*∂ᵣfrθθ/γrr + α + α*Krr*Kθθ/γrr
     - 2*α*frθθ^2/(γrr*γθθ) - α*frθθ*∂ᵣlnᾶ/γrr)

    @. ∂tfrrr = ( βʳ*∂ᵣfrrr - α*∂ᵣKrr - α*frrr*Krr/γrr
     + 12*α*frθθ*Kθθ*γrr/γθθ^2 - 10*α*frθθ*Krr/γθθ - 4*α*frrr*Kθθ/γθθ
     - α*Krr*∂ᵣlnᾶ - 4*α*Kθθ*γrr*∂ᵣlnᾶ/γθθ + 3*∂ᵣβʳ*frrr + γrr*∂ᵣ2βʳ )

    @. ∂tfrθθ = ( βʳ*∂ᵣfrθθ - α*∂ᵣKθθ - α*frrr*Kθθ/γrr + 2*α*frθθ*Kθθ/γθθ
     - α*Kθθ*∂ᵣlnᾶ + ∂ᵣβʳ*frθθ )
    ...
    @. ∂t𝜙 =   βʳ*∂ᵣ𝜙 - α*Π
    @. ∂tψ =   βʳ*∂ᵣψ - α*∂ᵣΠ - α*(frrr/γrr - 2*frθθ/γθθ + ∂ᵣlnᾶ)*Π + ψ*∂ᵣβʳ
    @. ∂tΠ = ( βʳ*∂ᵣΠ - α*∂ᵣψ/γrr + α*(Krr/γrr + 2*Kθθ/γθθ)*Π
     - α*(4*frθθ/γθθ + ∂ᵣlnᾶ)*ψ/γrr - m^2*𝜙 )
    ...
    if !(is_AH)
        ...
        ∂tUm𝜙 = @part 1 ( ∂tΠ - ∂tψ/sqrt(γrr) + ψ*∂tγrr/2/sqrt(γrr)^3 )
        Um𝜙   = @part 1 ( Π - ψ/sqrt(γrr) )

        ψrhs = ∂tψ[1]; Πrhs = ∂tΠ[1];

        # Dirichlet
        #∂tUp𝜙 = @part 1 ∂tUm𝜙*cm/cp + Um𝜙*(2*ᾶ*βʳ*∂tγθθ/cp^2)

        # Neumann
        ∂tUp𝜙 = @part 1 -(∂tUm𝜙*cm/cp)
        #∂tUp𝜙 = 0.
        ...
        # ∂tψ[1]  += s*sqrt(γrr[1])*(Up𝜙b - Up𝜙)/(dr̃*σ00)/2.
        # ∂tΠ[1]  += s*(Up𝜙b - Up𝜙)/(dr̃*σ00)/2.
        ...
        @part 1 ∂tΠ = ∂tUp𝜙/2 + Πrhs/2 - ψrhs/sqrt(γrr)/2 + ψ*γrrrhs/4/sqrt(γrr)^3
        @part 1 ∂tψ = ψrhs/2 + ∂tUp𝜙*sqrt(γrr)/2 - Πrhs*sqrt(γrr)/2 + ψ*γrrrhs/4/γrr
        ...
        #
        ∂tUmr = @part 1 ∂tKrr - ∂tfrrr/sqrt(γrr) + frrr*∂tγrr/2/sqrt(γrr)^3

        #∂tUpr = -∂tUmr
        ∂tUpr = 0.
        #∂tUpr = @part 1 ∂tKrr + ∂tfrrr/sqrt(γrr) - frrr*∂tγrr/2/sqrt(γrr)^3
        ...
        ∂tUpθ = @part 1 ( α - cp*∂ᵣUpθ + Umr*Upθ*α/γrr + (Upθ - Umθ)*Upθ*α/γθθ
    ...
    ∂tUmr = 0.
    #∂tUmr = @part n ∂tKrr - ∂tfrrr/sqrt(γrr) + frrr*∂tγrr/2/sqrt(γrr)^3
    ...
    ∂tUmθ = @part n ( α - cm*∂ᵣUmθ + Upr*Umθ*α/γrr
    ...
    #∂t𝜙[n]  += s*(U0𝜙b - U0𝜙)/(dr̃*σ00)/2
    @part n ( ∂t𝜙 = βʳ*ψ - α*Π )

    #∂t𝜙[n]  += s*(U0𝜙b - U0𝜙)/(dr̃*σ00)
    ∂tψ[n]  += -s*sqrt(γrr[n])*(Um𝜙b - Um𝜙)/(dr̃*σ00)/2.
    ∂tΠ[n]  += s*(Um𝜙b - Um𝜙)/(dr̃*σ00)/2.
...
function solution_saver(T,grid,sol,param,folder)
    ...
    "∂tΠ","H","Mr","Crrr","Crθθ","C𝜙","E","appHorizon"])
    varlen = length(vars)
