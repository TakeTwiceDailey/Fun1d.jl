...
@views function acoustic3D()
    ...
    t          = 0.0               # physical time
    # Numerics
    nx, ny, nz = 279,279,279     # numerical grid resolution; should be a mulitple of 32-1 for optimal GPU perf
    ...
    for it = 1:nt
        ...
        # Visualisation
        if mod(it,nout)==0
            heatmap(X, Z, Array(P)[:,y_sl,:]', aspect_ratio=1, xlims=(X[1],X[end]), ylims=(Z[1],Z[end]),clim=(-0.01,0.01), c=:viridis, title="Pressure"); frame(anim)
        end
    end
